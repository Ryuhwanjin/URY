...
class UnifiedDashboardApp:
    def __init__(self, root):
        self.root = root
        self.root.title(f"URY — Academic Studio {update_checker.CURRENT_VERSION}")
...
URY는 사용자의 로컬 컴퓨터 내에서만 독립적으로 동작하며, 어떠한 강의 음성이나 자료도 중앙 서버에 수집·보관하지 않습니다. AI 결과물은 학업 보조용 조교일 뿐 최종 시험 평가 기준을 대체하지 않습니다."""
    ...
    def create_header_card(self):
        ...
        tk.Label(title_row, text=f" {update_checker.CURRENT_VERSION}", font=("Pretendard", 9), bg="#ffffff", fg="#64748b").pack(side=tk.LEFT)
    ...
    def on_tab_changed(self, event=None):
        ...
        try:
            ...
            if any(k in current_tab for k in ("조교", "튜터", "Q&A")) and hasattr(self, "on_tutor_course_changed"):
    ...
    def refresh_course_combos(self):
        ...
        for combo_name in ("studio_course_combo", "exam_course_combo", "tutor_course_combo"):
    ...
    def build_update_tab(self):
        ...
        card.pack(fill=tk.X)
        ttk.Label(card, text=f"현재 버전  {update_checker.CURRENT_VERSION}", font=("Pretendard", 15, "bold"),
