    ...
    def test_requested_controls_and_tabs_exist_on_both_platforms(self):
        ...
        for platform in ("URY_macOS", "URY_Windows"):
            ...
            with self.subTest(platform=platform):
                ...
                self.assertIn("self._resize_guide_image = resize_guide_image", source)
                self.assertIn("min(1200, available_w)", source)
                self.assertIn("def _fit_window_size(width, height, screen_w, screen_h):", source)
                self.assertIn("min_w = min(700, available_w)", source)
                self.assertIn("min_h = min(500, available_h)", source)
                self.assertIn("gx = min(max(0, gx), max(0, sw - gw))", source)
                self.assertIn("gy = min(max(0, gy), max(0, sh - gh))", source)
                self.assertIn("tkfont.Font(root=self._font_root, font=self.font).measure(text)", source)
