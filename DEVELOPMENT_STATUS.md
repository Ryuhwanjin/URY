...
- Studio 강의 범위·중복 저장 보정 완료: 음성 발화 범위 우선 프롬프트와 동일 날짜 선택자료 교체 저장을 macOS·Windows에 동기화. 기존 2주차 파일은 두 번의 생성 결과가 합쳐진 사용자 산출물이라 자동 삭제하지 않았으며, v0.9.5 새 빌드에서 재생성하면 한 결과만 남음. 다음 명령: 상태 문서 포함 커밋·푸시·태그 후 v0.9.5 릴리즈 업로드
