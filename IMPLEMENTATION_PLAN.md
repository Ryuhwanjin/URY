...
- 최신 소스 커밋: `작업 중`
- 정식 배포: macOS `v0.9.1` 완료, 후속 `v0.9.2` 준비 중 (API 수동 재확인·Dock 아이콘 보정)
...
후속 `v0.9.2`에는 Settings의 API 연결 수동 재확인 1개 기능과 Dock 아이콘 안전 여백 보정만 포함한다. 새 생성 기능이나 대규모 UI 변경은 넣지 않는다.
