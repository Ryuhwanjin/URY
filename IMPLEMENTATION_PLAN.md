...
- 자동 테스트: 26개 통과, 1개 환경 의존 제외
- macOS 앱: 최신 스트리밍·8,192토큰 최적화 포함 빌드 완료
...
2. `업로드 → 인덱싱 → 서버 대기 → 스트림 수신 → 저장 → PDF` 단계별 시간을 로그에 기록
3. Studio 로그를 `system/logs/studio_latest.log`에 저장하고 API Key는 절대 기록하지 않음
4. 같은 파일의 Gemini File URI를 지문과 만료시간 기준으로 재사용
5. 429 응답을 RPD와 RPM/TPM으로 구분하고 사용자 행동을 정확히 안내
6. 취소 시 새 결과가 저장되거나 이전 실행과 섞이지 않는지 검증
...
2. User Guide에 실제 앱 스크린샷 삽입
3. 설치 후 앱 위치와 사용자 워크스페이스를 이동해 상대경로 시험
4. 테스트 릴리스 `v0.9.1-test` 정리
...
우선 `Phase 1 + Phase 2`만 승인하는 것을 권장한다. macOS 생성 안정성과 정식 릴리스를 확정한 뒤 Phase 3 Windows 작업을 시작하면 플랫폼 문제와 Gemini 문제를 동시에 디버깅하지 않아도 된다.
