...
# -*- coding: utf-8 -*-
"""
학업 관리 시스템 중앙 설정 관리자 v0.7.7 (Config Manager)
...
def get_root_workspace():
    ...
    try:
        ...
        if dev_src != user_ws and os.path.exists(dev_sem):
            import shutil
            for root, dirs, files in os.walk(dev_sem):
                for f in files:
                    if f.endswith(".pdf"):
                        sp = os.path.join(root, f)
                        rp = os.path.relpath(sp, dev_sem)
                        dp = os.path.join(user_ws, "2026년 2학기", rp)
...
def set_course_syllabus(course_name_or_folder, file_paths):
    """
    과목의 강의계획서 경로 목록을 settings.json에 저장 (단일 str 또는 list 모두 허용)
    """
    ...
    for c in settings.get("courses", []):
        if c.get("course_name") == course_name_or_folder or c.get("folder_name") == course_name_or_folder:
            c["syllabus_paths"] = file_paths
            c["syllabus_path"] = file_paths[0] if file_paths else ""
            updated = True
            break
...
def parse_model_version_score(name):
    ...
    return ver_score + tier_score + special_penalty
...
def create_sample_test_files():
    ...
    first_course = courses[0].get("course_name", "마케팅원론")
