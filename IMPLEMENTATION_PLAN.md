...
- Windows v0.9.9 유지보수 후보: Run 28의 첫 수정은 기존 Markdown 접근 문제에만 적용되어 첫 실행의 새 Markdown 생성 오류를 해결하지 못했다. 워크스페이스 쓰기 권한을 실제 파일 생성으로 검사하고 Windows 사용자 데이터 폴더로 전환하는 보완을 구현했다. 선택한 대체 경로는 이후 실행과 완전 삭제에서도 유지된다. Run 30의 ZIP·설치 EXE 빌드는 성공했다. 새 폴더 첫 저장·두 후보 경로 모두 거부·명시적 사용자 워크스페이스 테스트를 추가하고 Windows CI가 전체 unittest를 실행하도록 변경했으며, 네이티브 CI와 Windows 실기기 확인 후 공개 릴리즈한다.
...
6. GitHub Actions Windows runner 기반 v0.9.8 `--onedir` 빌드·아이콘·번들 리소스 Verify 및 `installer/URY_v0.9.8.iss` Inno Setup 패키지를 완료. v0.9.9 첫 실행 쓰기 권한 fallback을 포함한 ZIP·설치 EXE는 Run 30에서 빌드했다. 이후 workflow가 패키징 전 전체 unittest를 실행하도록 보강했으며 네이티브 runner 결과와 실제 Windows 설치·기능 QA가 남아 있다.
