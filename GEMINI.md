...
- 최신 기준은 `git log -1 --oneline`과 세 문서의 마지막 갱신일로 확인한다. macOS·Windows v0.9.7 정식 릴리즈까지 완료되어 Windows Phase 3 실기기 QA를 진행한다.
- Studio 생성은 Gemini SSE 스트리밍, 최대 출력 8,192토큰, 대기 제한 240초를 사용하며, 음성 발화 범위 제한·동일 날짜 교체 저장·503/429 모델 fallback을 포함한다.
- 최신 공개 릴리즈는 `https://github.com/Ryuhwanjin/URY/releases/tag/v0.9.7`이며 macOS DMG/ZIP, Windows setup EXE/onedir ZIP 4개 asset과 플랫폼별 업데이트 인식을 확인했다. Windows 실기기 설치·업데이트·삭제 QA가 남아 있다.
