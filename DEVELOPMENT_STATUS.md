...
- 플랫폼별 업데이트 인식 수정의 v0.9.7 배포 빌드:
  - 대상 파일: macOS 앱·DMG·ZIP, Windows EXE·설치파일·ZIP, GitHub Release asset
  - 현재 상태: v0.9.7 버전 표기 갱신, macOS 앱·DMG·ZIP 빌드 완료. Windows v0.9.7 Actions 빌드 실행 대기
  - 바로 다음 명령: 버전 갱신 커밋을 push하고 Windows Actions의 EXE·설치파일 artifact 생성 결과 확인
...
- Windows runner artifact 검증 완료: `13606a6` 기준 `URY.exe` 빌드 성공, `0fd46d0` 기준 아이콘·번들 프롬프트 Verify·ZIP artifact 성공. `f094eff` 기준 Inno Setup 설치 스크립트(`installer/URY_v0.9.6.iss`)와 `URY_Windows_v0.9.6_setup`(약 40.5MB)·`URY_Windows_v0.9.6_onedir`(약 53.4MB) artifact 생성 성공. 실제 설치 QA 대기
- 저장소 구조 정리 완료: 미참조 아이콘·보안 우회 스크립트·수동 전체 파이프라인 런처와 dead wrapper를 제거하고, macOS launcher의 자동 quarantine 해제를 없앴다. README·DMG/릴리즈 빌드 안내를 갱신했으며 전체 테스트 44개 통과(1개 환경 의존 제외), 커밋 `363d072`. 정리 후 Windows CI 재빌드·Inno Setup까지 성공(Run `34550568457`)
- Studio 주차·통합 적재 및 영문 부분 응답 보정 완료: `session_only` 생성도 날짜별 원본과 함께 `N주차`·`통합` Markdown을 갱신하고, 같은 날짜 재생성은 해당 섹션을 교체. 18토큰/200자처럼 4개 섹션이 없는 스트리밍 응답은 저장하지 않고 다음 모델로 전환하며, 영문 실패 시 잘린 파일 없이 한국어 결과를 유지. macOS·Windows 공용 코드 동기화, 테스트 38개 통과(1개 환경 의존 제외), 구문·diff 검증 완료. 커밋 `6d06e56`
