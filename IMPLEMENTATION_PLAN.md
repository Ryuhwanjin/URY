...
- **저작권·음성 녹음 위험 완화 및 고지**:
  - URY 자체 서버에는 사용자 자료를 저장하지 않는다. 다만 사용자가 선택한 음성·문서는 사용자의 Gemini API Key를 통해 Google API로 전송될 수 있으며, 해당 서비스의 이용약관·보존정책·학교 규정을 따른다.
  - 개인 학습 목적의 사용을 전제로 안내하지만, 저작권법 제30조 적용 여부나 녹음의 적법성은 구체적 사실관계와 관할 규정에 따라 달라지므로 법적 면책으로 단정하지 않는다.
...
- **버전 정책 (안 B 채택)**:
  ...
  - 플랫폼별 업데이트 인식 수정은 기존 v0.9.6 설치본이 실제로 업데이트받을 수 있도록 유지보수 버전 `v0.9.7`로 먼저 배포한다.
...
- Windows v0.9.9 유지보수 후보: Run 28의 첫 수정은 기존 Markdown 접근 문제에만 적용되어 첫 실행의 새 Markdown 생성 오류를 해결하지 못했다. 워크스페이스 쓰기 권한을 실제 파일 생성으로 검사하고 Windows 사용자 데이터 폴더로 전환하는 보완을 구현했다. 선택한 대체 경로는 이후 실행과 완전 삭제에서도 유지된다. Actions Run 30의 ZIP·설치 EXE 빌드는 성공했으며 Windows 실기기 확인 후 공개 릴리즈한다.

- 최신 기준: v0.9.8 (v0.9.7 `97457ed` 기반 녹음기 TCC 권한 수정)
- 배포 정책 확정: 비영리 100% 무료 배포(무광고), 버전 정책 안 B(v0.9.6 검증 후 v1.0.0 승격)
- 의사결정 권한: GPT(Codex)가 구현·검증·병합·릴리즈를 최종 결정하고, Antigravity는 더블체크와 아이디어 제안만 수행
...
- macOS 앱: 최신 스트리밍·8,192토큰 최적화·503 fallback·음성 범위·중복 저장 보정 포함 v0.9.5 후보 빌드 완료. v0.9.6은 주차·통합 적재 및 부분 응답 차단 패치 릴리즈 준비 중
...
13. Studio의 날짜별 생성 결과는 해당 `N주차` 파일과 `통합` 파일에도 즉시 병합하고, 4개 섹션이 없는 부분 응답은 저장하지 않음 — macOS·Windows 공용 코드·회귀 테스트 반영 완료
...
7. 업데이트 알림은 GitHub Release 전체 태그가 아니라 현재 플랫폼에 맞는 DMG 또는 EXE/ZIP asset 버전만 비교한다.
...
6. GitHub Actions Windows runner 기반 v0.9.8 `--onedir` 빌드·아이콘·번들 리소스 Verify 및 `installer/URY_v0.9.8.iss` Inno Setup 패키지를 완료. v0.9.9 첫 실행 쓰기 권한 fallback을 포함한 ZIP·설치 EXE는 Run 30에서 빌드했다. 실제 Windows 설치·기능 QA가 남아 있다.
